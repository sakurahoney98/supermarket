...
int columns = 0;
	            ...
	            // Adiciona uma tabela
	            PdfPTable table = new PdfPTable(columns); // 4 colunas
	        	...
	        	if(categories.size() > 1) {
		        	...
		        	for(int j = 0; j < list_1.size() || j < list_2.size(); j++) {
		        		if(j < list_1.size()) {
		        			...
		        			if(product.getDuration() < 30) {
		        				buy =  (int) Math.ceil(30 / (double) product.getDuration()) - product.getStock_quantity();
		        			...
		        			table.addCell(String.valueOf(buy > product.getMax_purchase() ? product.getMax_purchase() : buy) + "x" + product.getQuantity());
	        		...
	        		for(int j = 0; j < list_1.size(); j++) {
	        			...
	        			if(columns > 2) {
		        			...
		        			table.addCell(String.valueOf(buy > product.getMax_purchase() ? product.getMax_purchase() : buy) + "x" + list_1.get(j).getQuantity());
